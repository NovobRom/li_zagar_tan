...
- [x] **1.1.1** Добавить редирект в `admin/layout.tsx` если user не авторизован:
  ```typescript
  if (!user) {
    redirect('/admin');
  }
  ```
- [x] **1.1.2** Вызывать `supabase.rpc('is_admin')` в каждом защищённом action
- [x] **1.1.3** Добавить проверку роли перед операциями с галереей
...
- [x] **1.2.1** Использовать Zod для валидации FormData:
  ```typescript
  ...
  ```
- [x] **1.2.2** Проверять расширение файла из whitelist: `['jpg', 'jpeg', 'png', 'webp']`
- [x] **1.2.3** Перед удалением проверять существование фото и совпадение `storage_path`
- [x] **1.2.4** Добавить regex валидацию email и минимальную длину пароля
...
- [x] Установить и настроить Zod для всех Server Actions
- [x] Создать общие схемы валидации в `app/lib/validation.ts`
- [x] Валидировать все FormData перед обработкой
- [x] Проверять MIME-типы файлов на сервере (не только расширение)
...
- [x] Не отправлять технические детали ошибок клиенту
...
| Сохранение выбора языка | ✅ Работает (Cookie + LocalStorage) | 100% |
| Динамические мета-теги | ✅ Реализовано | 100% |
| hreflang атрибуты | ❌ Отсутствуют | 0% |
| HTML lang атрибут | ✅ Динамический | 100% |
...
- [x] **Динамические мета-теги** — использовать `generateMetadata()` в layout.tsx:
  ```typescript
...
- [x] **Сохранение языка** — использовать localStorage (и cookies):
  ```typescript
  useEffect(() => {
...
- [x] Создать локализованные мета-теги для каждого языка
- [ ] Добавить canonical URL
- [x] Исправить Open Graph теги для соцсетей
...
- [x] Обновить зависимости:
  ...
  ```
- [x] Исправить lint скрипт: `"lint": "eslint . --ext .ts,.tsx"`
- [x] Добавить скрипты:
  ```json
...
1. **Немедленно** (сегодня):
   - [x] Исправить авторизацию в `admin/layout.tsx` (перенесено в `admin/dashboard/layout.tsx`)
   - [x] Создать `tailwind.config.ts`
   - [x] Обновить зависимости (`npm update`)

2. **Эта неделя**:
   - [x] Добавить серверную валидацию во все Server Actions
   - [x] Реализовать сохранение языка в localStorage
   - [x] Добавить динамические мета-теги

      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-4 sm:gap-6 lg:grid-cols-3 lg:gap-8 max-w-lg mx-auto lg:max-w-none">
          {services.map((service, index) => (
            <div
              key={index}
              className={`relative bg-white rounded-2xl sm:rounded-3xl shadow-lg overflow-hidden ${
                service.popular ? 'ring-2 ring-amber-500 lg:scale-105' : ''
              }`}
            >
              {/* Popular Badge */}
              {service.popular && (
                <div className="absolute top-0 right-0 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs font-bold px-3 py-1 rounded-bl-xl">
                  {t.pricing.popular}
                </div>
              )}
              ...
              <div className="p-5 sm:p-6 lg:p-8">
                {/* Service Name */}
                <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2 pr-16 sm:pr-0">
                  {service.name}
                </h3>
                ...
                <a href="#booking" className="block">
                  <button
                    className={`w-full py-3 px-6 rounded-full font-semibold text-sm sm:text-base transition-all duration-200 active:scale-[0.98] ${
                      service.popular
